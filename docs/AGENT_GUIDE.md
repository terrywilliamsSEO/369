...
The current best 3 -> 6 -> 9 candidates still fail 4x promotion. Stage A `+0.03` tuning can remove target slips, and forensics found budget-clean zero-slip compensation rows. The refined-dt basin check preserved budget cleanliness, but lock/jump/envelope gates still failed. The limiter/predictive-servo smoke improved 369 lock above 0.96 with passive adaptive damping, but generated-envelope CV stayed around 0.28 and max phase jumps stayed around 1.75 rad, so no row promoted. The harmonic-family smoke found 5 -> 10 -> 15 strongest by normalized passive score and 4 -> 8 -> 12 closest to a candidate, while 3 -> 6 -> 9 still failed generated-envelope and jump gates. Next work should treat f->2f->3f family-law mapping as first-class before any geometry/evolve or 369-specific PLL branch.
