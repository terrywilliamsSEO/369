...
```bash
python spice_412_hybrid_magnetic_refine.py --run --ngspice-path wsl:ngspice --timeout 180
```

Outputs:

- `runs/spice_412_hybrid_magnetic_refine/spice_412_hybrid_magnetic_refine_summary.json`
- `runs/spice_412_hybrid_magnetic_refine/spice_412_hybrid_magnetic_refine_summary.csv`
- `runs/spice_412_hybrid_magnetic_refine/spice_412_hybrid_magnetic_refine_timeseries.csv`
- `runs/spice_412_hybrid_magnetic_refine/README_SPICE_412_HYBRID_MAGNETIC_REFINE.md`

What it tests:

- Focused refinement of the strongest electrical race family: hybrid varactor-plus-magnetic transmission line.
- Sweep areas include section placement, magnetic saturation/loss/bias proxies, varactor swing/bias/stacking, phase-velocity trim, target extraction/rejection, output loading, and stress reduction.
- Discovery rows remain source-only at 50 MHz: no direct 100 MHz drive, no direct 150 MHz drive, no target-frequency injection, and no hidden behavioral target source.
- Controls include linear fixed-component, varactor-only, magnetic-only, weak hybrid, detuned 150 MHz velocity, shuffled frequency, too-short, too-lossy, phase-mismatched, target extraction without nonlinearity, hybrid nonlinearity without extraction, and a separated direct 50+100 MHz ceiling reference.

Standalone result:

- The final run evaluated 37 rows: 25 discovery rows, 11 controls, and one ceiling reference.
- 28 rows ran successfully under WSL `ngspice-42`; 9 aggressive discovery rows failed convergence or timeout.
- No `spice_hybrid_magnetic_412_candidate` promoted because no row reached 150 MHz purity above `0.80`.
- Three `spice_hybrid_magnetic_412_near_miss` rows promoted above the `0.30` purity gate.
...
- `runs/spice_412_electrical_control_forensics/spice_412_electrical_control_forensics_summary.json`
- `runs/spice_412_electrical_control_forensics/spice_412_electrical_control_forensics_summary.csv`
- `runs/spice_412_electrical_control_forensics/spice_412_electrical_control_forensics_timeseries.csv`
- `runs/spice_412_electrical_control_forensics/README_SPICE_412_ELECTRICAL_CONTROL_FORENSICS.md`

What it tests:

- Re-instruments the best hybrid lock-in rows, pure-varactor controls, pure-magnetic controls, generated-path suppression, target-velocity detuning, phase mismatch, linear extraction, and the separated direct 50+100 MHz ceiling reference.
- Writes internal quarter/mid/three-quarter/raw line probes plus post-extraction output so pre-extraction 150 MHz can be separated from readout/filter selectivity.
- Keeps source-only 50 MHz drive for all discovery/control rows: no direct 100 MHz drive, no direct 150 MHz drive, no target-frequency injection, and no hidden target-band behavioral source.

Standalone result:

- All 12 rows ran successfully under WSL `ngspice-42`.
- The hybrid does produce more pre-extraction 150 MHz than pure varactor: best hybrid pre-extraction purity was `0.703380`, versus tuned pure-varactor `0.229162`.
- Generated-path suppression killed the 150 MHz response with dependency score `0.999999825`; phase mismatch also killed it with kill score `0.886001`.
- The strongest post-extraction hybrid purity was `0.996095`, but extraction dominated apparent purity with filter selectivity `55.681793` on `f001`.
- Controls did not stay dead overall. The tuned pure-varactor extraction control reached post-extraction purity `0.948335`, pre-extraction growth `1.222894`, and leakage score `0.817293`; target-velocity detuned control also leaked with score `0.315297`.
...
Optional bounded mini-validation:
...
- `runs/spice_412_phase_slip_tomography/tomography_summary.json`
- `runs/spice_412_phase_slip_tomography/tomography_summary.csv`
- `runs/spice_412_phase_slip_tomography/tap_phase_error.csv`
- `runs/spice_412_phase_slip_tomography/local_growth_by_tap.csv`
- `runs/spice_412_phase_slip_tomography/rescue_plan.json`
- `runs/spice_412_phase_slip_tomography/rescue_plan.md`
- `runs/spice_412_phase_slip_tomography/README_SPICE_412_PHASE_SLIP_TOMOGRAPHY.md`

What it tests:

- Reads the existing local `runs/spice_412_differential_witness_line/` artifacts; it does not start with a new broad sweep.
- Reconstructs tap-by-tap complex phasors at 50, 100, and 150 MHz from successful transient CSVs.
- Computes unwrapped phase, local phase jumps, local amplitude growth, coherent-growth contribution, 100 MHz lock error `phi_100 - 2 phi_50`, 150 MHz lock error `phi_150 - phi_100 - phi_50`, QPM sign alignment, reflection/standing-wave indicators, and generated/target path stability.
- Classifies each pair as generated instability, target phase walkoff, QPM misalignment, magnetic lag, reflection/load behavior, shadow leakage, extraction artifact, raw gain without coherence, convergence-limited, or genuinely blocked.

Standalone result:

- Fifteen paired trials were analyzed; ten were successful paired trials and five remained convergence-limited from the witness run.
- No `electrical_phase_rescue_candidate` promoted.
- One relaxed `electrical_phase_rescue_near_miss` appeared: the low-frequency magnetic-line surrogate had gain `64.3243730813572`, target lock `0.9533687649119508`, and coherent growth `1.9880897647161182`, but it still showed target phase walkoff, reflection/load behavior, and target-envelope CV about `0.484`, so it does not unblock the topology.
...
- Closest raw-gain pair was `no_qpm_baseline__vs__linear_no_nonlinearity_shadow` with gain `70816.74618476344`, but target lock was only `0.5196591492317262`; 100 MHz and 150 MHz coherence both failed by `tap_1_8`.
- The best braided-object pairs also failed by `tap_1_8`, with target lock near `0.757` and coherent growth below `0.80`.
...
- `runs/acoustic_412_sacred_modal_triads/summary.json`
- `runs/acoustic_412_sacred_modal_triads/summary.csv`
- `runs/acoustic_412_sacred_modal_triads/modal_triads.csv`
- `runs/acoustic_412_sacred_modal_triads/promoted_modal_geometries.csv`
- `runs/acoustic_412_sacred_modal_triads/matched_controls.csv`
- `runs/acoustic_412_sacred_modal_triads/failure_modes.csv`
- `runs/acoustic_412_sacred_modal_triads/README_ACOUSTIC_412_SACRED_MODAL_TRIADS.md`
- SVG diagrams for best modal triad geometry, best failed row, strongest leaking control, and any promoted row.

What it tests:

- Sacred-geometry-inspired graph names only as acoustic topology candidates, not as proof of sacred geometry or crop-circle messages.
- Ten geometries: Sri Yantra nested triangles, Metatron's cube, vesica piscis / seed of life, Fibonacci phyllotaxis, golden-angle spiral, Penrose-like quasicrystal patch, dodecagonal 12-fold quasicrystal, hexagram dual-triangle lattice, tree-of-life graph, and synthetic Chladni template.
- Stage 1 computes graph Laplacian acoustic modes, maps them to approximate 40/80/120 kHz modal triads, and scores A+A->B overlap, A+B->C overlap, source coupling, target localization, mode separation, geometry dependency, and modal-control leakage.
- Stage 2 runs nonlinear time simulation only for the top modal-triad geometry bundles with source-only 40 kHz drive, no direct 80 kHz drive, no direct 120 kHz drive, and no target-frequency injection.
...
- Aggregate label: `not_promoted`; decision: `no_hardware_build_readiness`.
- Best modal row was `m010 chladni_template_synthetic`: triad ratio error `0.024541`, overlap_448 `0.116918`, overlap_4812 `0.009900`, source coupling `0.485069`, target localization `0.121439`, geometry dependency score `0.0`, and modal control leakage `1.0`.
...
- Controls did not stay dead: 57 Stage 1 modal controls leaked and 22 Stage 2 nonlinear controls leaked. Strongest modal leak was `m001_radial_distribution_angle_shuffle`; strongest Stage 2 leak was `m010_missing_key_nodes`.
- Current interpretation: modal triads alone are not enough. This pass does not prove sacred geometry and does not rescue the acoustic route; continuation would need both cleaner modal dependency and real raw nonlinear 120 kHz purity/growth.
...
4. Treat sacred-modal graph layouts as failed in this first modal-triad model; only continue if the redesign improves modal dependency and Stage 2 raw 120 kHz purity/growth together.
5. Pause the current electrical topology behind the acoustic branch; only continue electrical work through a different topology, independent readout, or more physical nonlinear magnetic-line model that can beat its matched witness shadow before extraction and preserve tap-level coherence.
6. Run the expanded `harmonic_bridge_412_detuning_refine --quick --sweeps` grid when runtime is acceptable.
7. Treat the entire f->2f->3f family as first-class until 369 beats it under normalized budget scoring.
8. If staying on 369, use either a true PLL or a more physical limiter redesign; predictive timing alone did not clear jump/CV gates.
9. Add a geometry/evolve mode only after a 4x-stable 3 -> 6 -> 9 seed beats non-369 controls under the same accounting.
